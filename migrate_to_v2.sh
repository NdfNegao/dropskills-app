...
# Exécution sécurisée avec points de contrôle
# =====================================================
...
# Fonction pour exécuter du SQL avec gestion d'erreur
execute_sql() {
    local file=$1
    local description=$2
    ...
    if psql "$DATABASE_URL" -f "$file" > "logs/${file%.sql}_$(date +%Y%m%d_%H%M%S).log" 2>&1; then
...
echo "🚀 MIGRATION DROPSKILLS VERS V2"
echo "================================"
echo ""
...
execute_sql "audit_current_db.sql" "Audit de la base de données actuelle"

# Étape 3: Création du schéma V2
log "=== ÉTAPE 3: CRÉATION DU SCHÉMA V2 ==="
execute_sql "create_v2_schema.sql" "Création du nouveau schéma V2"
...
execute_sql "temp_migration.sql" "Migration des données vers V2"
...
execute_sql "validate_migration.sql" "Validation de la migration"
...
SET search_path TO dropskills_v2;
SELECT '- **' || table_name || ':** ' || count || ' enregistrements'
FROM (
  SELECT 'Users' as table_name, COUNT(*) as count FROM users
  UNION ALL
  SELECT 'Packs' as table_name, COUNT(*) as count FROM packs
  ...
  SELECT 'Samples' as table_name, COUNT(*) as count FROM samples
  UNION ALL
  SELECT 'Favoris' as table_name, COUNT(*) as count FROM favorites
  UNION ALL
  SELECT 'Tags' as table_name, COUNT(*) as count FROM tags
  UNION ALL
  SELECT 'User Packs' as table_name, COUNT(*) as count FROM user_packs
  UNION ALL
  SELECT 'Trending Ideas' as table_name, COUNT(*) as count FROM trending_ideas
  UNION ALL
  SELECT 'Outils IA' as table_name, COUNT(*) as count FROM ai_tools
) t;
SET search_path TO public;
")
